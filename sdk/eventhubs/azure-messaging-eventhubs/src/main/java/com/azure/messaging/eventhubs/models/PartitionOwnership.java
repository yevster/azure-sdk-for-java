...
public class PartitionOwnership {
     ...
     * achieve optimistic concurrency in a distributed event processor setup. When multiple instances of event
     * processor
     * try to update the same partition ownership record, ETag is used to verify that the last values read by the
     * instance
     * requesting the update is still the latest ETag for this record. If the ETag in the store does not match the ETag
     * in
